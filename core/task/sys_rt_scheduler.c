...
#include "sys_error.h"
void sys_rt_scheduler_init(sys_rt_scheduler_t *rt_scheduler)
{
    sys_trace();
...
void sys_rt_task_control_block_init(sys_rt_scheduler_t *rt_scheduler, sys_rt_task_control_block_t *rt_task_control_block, int priority)
{
    sys_trace();
...
int sys_rt_scheduler_set_priority(sys_rt_scheduler_t *rt_scheduler, sys_rt_task_control_block_t *rt_task_control_block, int priority)
{
    sys_trace();
    if (rt_task_control_block->priority < 0 || rt_task_control_block->priority >= SYS_RTSCHED_MAX_PRIORITY)
    {
